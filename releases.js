...
async function fetchAndWriteLatestReleases() {
  ...
  while (stableReleaseCount < MIN_COUNT_STABLE_RELEASES || betaReleaseCount < MIN_COUNT_BETA_RELEASES) {
...
async function fetchLatestReleases(page) {
  const pageIndex = page != null && page > 0 ? page : 1;
...
function loadLatestReleases() {
  ...
  const releases = data.filter((x) => !x.draft && x.name != null && !x.name.includes('-pre'));
